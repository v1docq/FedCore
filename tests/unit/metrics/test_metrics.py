...
# def test_metrics_factory_():
#     target = torch.Tensor([1, 1,])
#     preds = torch.Tensor([[0, 1], [0.2, 0]])
#     name_suffix = 'MulticlassPrecision__2'
#     mtr = MetricFactory.get_metric(name_suffix)
#     assert name_suffix in LOADED_METRICS, "For each combination metric - type the corresponding class should be created"
#     assert MetricFactory.get_metric(name_suffix) is mtr, "In case the previously called metric is used, the existing class should be summoned"
#     assert mtr.metric(target, preds) is not None, 'wrong `.metric` logic'
#     assert mtr.metric(target, torch.argmax(preds, -1)) is not None, 'wrong `.metric` logic'

#     name_suffix = 'MulticlassAUROC__2'
#     mtr = MetricFactory.get_metric(name_suffix)
#     assert mtr.metric(target, preds) is not None, 'wrong `.metric` logic. the type-based decorator didn\'t cope with the input'
