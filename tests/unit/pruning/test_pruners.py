...
def test_pruners(pruner_name):
    ...
    before_pruning = sum(p.numel() for p in train_data.target.parameters())
    after_pruning = sum(p.numel() for p in pruned_model.parameters())

    assert before_pruning > after_pruning, f'{pruner_name} pruner doesnt reduce number of model parameters'
