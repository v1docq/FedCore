...
N_TREES = 10000
VERBOSE = 100
DATASET = dict(rt_iot='https://archive.ics.uci.edu/dataset/942/rt-iot20221', # 68,458 rows × 86 columns
               yeast='https://archive.ics.uci.edu/dataset/110/yeast' # 1,484 rows × 8 columns
               )
sketch = RandomProjectionSketch(1)
BOOSTING_MODEL_PARAMS = dict(loss='crossentropy', ntrees=N_TREES, lr=0.03, verbose=VERBOSE,
                             es=300, lambda_l2=1, gd_steps=1, subsample=1, colsample=1,
                             min_data_in_leaf=10, max_bin=256, max_depth=6, multioutput_sketch=sketch)
...
def use_fedcore_pyboost(X, X_test, y, y_test):
    model = FedcoreBoostingModel(
        loss='crossentropy',
        ntrees=N_TREES, lr=0.03, verbose=VERBOSE, es=300, lambda_l2=1, gd_steps=1,
        subsample=1, colsample=1, min_data_in_leaf=10,
        max_bin=256, max_depth=6,
...
ind = eval_boosting_perfomance(use_fedcore_pyboost, X, X_test, y, y_test)
pyboost_default = eval_boosting_perfomance(use_default_pyboost, X, X_test, y, y_test)
pyboost_fedcore = eval_boosting_perfomance(partial(use_default_pyboost, use_random=True), X, X_test, y, y_test)
_ = 1
