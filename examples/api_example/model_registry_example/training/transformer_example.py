...
log_file = os.path.join(log_dir, 'training_transformer.log')
...
def create_training_config(model, tokenizer, fedcore_id=None):
    """Create training configuration using LLMTrainer."""
...
def create_api_config(model, tokenizer, fedcore_id=None):
    """Create API configuration with training strategy."""
    training_config = create_training_config(model, tokenizer, fedcore_id)
    ...
    compute_config = ComputeConfigTemplate(
        output_folder='./llm_training_experiment',
        automl_folder='./llm_training_automl'
    )
