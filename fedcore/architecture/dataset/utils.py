...
def load_from_tsfile_to_dataframe(
    ...
    encoding = predict_encoding(full_file_path_and_name)
...
def read_tsv_or_csv(data_path: str, mode: str = 'tsv') -> tuple:
    ...
    return load_process_data(data_path, separator)


def read_txt_files(data_path: str):
    """
...
def read_ts_files(data_path: str):
    """
    ...
    features, target = load_process_data(data_path)
    list_of_dimension = features.columns
    multidim_array = []
    for dim in list_of_dimension:
        all_samples_at_dim = features[dim].values #pd.Series format
        list_for_arr = [sample.values for sample in all_samples_at_dim]
        multidim_array.append(np.stack(list_for_arr))
    features = np.stack(multidim_array)
    return features, target
