...
from fedcore.repository.constanst_repository import PRUNERS, PRUNING_IMPORTANCE, \
    PRUNER_REQUIRED_REG, PRUNER_WITHOUT_REQUIREMENTS, PRUNING_FUNC, MANUAL_PRUNING_STRATEGY
...
class BasePruner(BaseCompressionModel):
    ...
    def _init_model(self, input_data):
        ...
        self.ignored_layers = self.validator.filter_ignored_layers(self.model, str(self.model.__class__))
    ...
    def _define_root_layer(self):
        root_layer_dict = {'manual_conv': self.optimised_model.conv1, }
    ...
    def _dependency_graph_pruner(self):
        DG = tp.DependencyGraph().build_dependency(self.optimised_model,
                                                   example_inputs=self.data_batch_for_calib,
                                                   ignored_layers=self.ignored_layers)
        group = DG.get_pruning_group(self.optimised_model.conv1, tp.prune_conv_out_channels, idxs=[2, 6, 9])
    ...
    def _manual_pruning_iter(self, pruner):
        for i in range(self.pruning_iterations):
            for pruning_func in MANUAL_PRUNING_STRATEGY[self.pruner_name]:
                ...
                pruner.manual_prune(root_layer, PRUNING_FUNC[pruning_func], self.pruning_ratio)
    ...
    def compress(self, input_data: InputData) -> np.array:
        ...
        base_macs, base_nparams = tp.utils.count_ops_and_params(self.model, self.data_batch_for_calib)
        manual_pruning = self.pruner_name.__contains__('manual')

        self.pruner = self.pruner_step(self.pruner, input_data)
        self.pruner = Either(value=self.pruner,
        ...
        print("==============After pruning=================")
        macs, nparams = tp.utils.count_ops_and_params(self.optimised_model, self.data_batch_for_calib)
