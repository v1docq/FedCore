...
class LowRankModel:
    ...
    def __init__(
        ...
        self.energy_thresholds = params.get('epochs', ENERGY_THR)
        self.decomposing_mode = params.get('epochs', DECOMPOSE_MODE)
        self.forward_mode = params.get('epochs', FORWARD_MODE)
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for batch in tqdm(train_loader):
                ...
                if custom_loss:
                    ...
                    loss_sum += sum([loss.item() for loss in model_loss.values()]) + quality_loss.item()
                else:
    ...
    def fit(
        ...
        self.optimize(model=self.model, params=input_data.features, ft_params=input_data.features)
        ...
        for thr in self.energy_thresholds:
            for name, module in model.named_children():
                if isinstance(module, DecomposedConv2d):
                    rank_threshold_pruning(module, thr)
            if ft_params is not None:
                pass
                # train(train_loader=params['train_dataset'], net=model, epochs=params['epoch'],
                #       custom_loss=self.__loss())
