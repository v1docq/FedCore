...
class QuantPostModel(BaseCompressionModel):
    ...
    def fit(self,
        ...
        self.model = input_data.target if 'predict' not in vars(input_data) else input_data.predict
        self.optimized_model = self.quantisation_model.fit(model=self.model,
