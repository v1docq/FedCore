...
class FedCoreTransformersTrainer(Trainer):
        ...
        for hook_enum in hook_enums:
            for hook_elem in hook_enum:
                try:
                    ...
                    print(f"[Warning] Failed to initialize hook {hook_elem}: {e}")
                    continue
        ...
        if trainer:
            for hook in self.hooks_collection.end:
                hook(
                    ...
                    trainer_objects=self.trainer_objects,
                    history=self.history,
                    val_loader=trainer.get_eval_dataloader(trainer.eval_dataset) if trainer.eval_dataset else None,
    ...
    def create_optimizer(self):
        ...
        self.trainer_objects['optimizer'] = optimizer  # Сохраняем созданный оптимизатор
        return optimizer
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        self.trainer_objects['scheduler'] = scheduler  # Сохраняем созданный оптимизатор
        return scheduler