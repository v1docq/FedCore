...
from fedcore.repository.constanst_repository import RMSE, TorchLossesConstant
...
from fedcore.models.network_modules.layers.linear_layers import Flatten, Transpose
from fedcore.models.network_modules.layers.padding_layers import Pad1d
...
class _TSTEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        # Multi-Head attention
        src2, attn = self.self_attn(Q=src, K=src, V=src, attn_mask=mask)
        # Add & Norm
...
class TimeSeriesTransformerTorch(Module):
    ...
    def forward(self,
        ...
        # Input encoding
        if self.new_q_len:
            u = self.W_P(x).transpose(2, 1)  # Eq 2
            # u: [bs x model_dim x q_len] transposed to [bs x q_len x
            # model_dim]
        else:
            u = self.W_P(x.transpose(2, 1))  # Eq 1
        ...
        # Encoder
        z = self.encoder(u, mask)  # z: [bs x q_len x model_dim]
        z = z.transpose(2, 1).contiguous()  # z: [bs x model_dim x q_len]

        # Classification/ Regression head
        return self.head(z)

class TSTModel(BaseNeuralForecaster):
