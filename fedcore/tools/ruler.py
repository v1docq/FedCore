...
class PerformanceEvaluator:
    def __init__(self, model, data, device=default_device(), batch_size=32, n_batches=8, collate_fn=None):
    ...
    def throughput_eval(self, num_iterations=30):
        ...
        for batch, _ in tqdm(self._preloaded_batches_gen(self.n_batches), desc='batches', unit='batch'):
            X = batch.cuda(non_blocking=True) if hasattr(batch, 'cuda') else batch.to(self.device)
            batch_size = len(X) 
    ...
    def latency_eval(self, max_samples=None):
        ...
        for batch, _ in tqdm(self._preloaded_batches_gen(max_samples or self.batch_size)):
            batch = batch if hasattr(batch, '__iter__') else [batch] ### case batch is not iterable
            for sample in batch:
                sample = sample.cuda(non_blocking=True) if hasattr(sample, 'cuda') else sample.to(self.device)
                tic1 = time.time()
                self.model(sample)
