...
def _get_absorb_per_block(model, example_inputs, folding=False, weight_config={}):
    ...
    for k, v in absorb_to_layer.items():
        for vv in v:
            if vv in weight_config and (
                weight_config[vv]["algorithm"] != "AWQ"
                or weight_config[vv]["bits"] == -1
            ):
                skip_op_set.add(k)
    for k in no_absorb_layers:
        if k in weight_config and (
            weight_config[k]["algorithm"] != "AWQ" or weight_config[k]["bits"] == -1
        ):
            skip_op_set.add(k)
