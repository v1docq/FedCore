...
class QuantizeGraphHelper:
    ...
    def get_sorted_graph(self, input_graph, input_node_names, output_node_names):
        ...
        all_ops = [
            i for i in list(self.node_name_mapping.keys()) if i not in self.op_list
        ]
    ...
    def split_shared_inputs(input_graph_def):
        ...
        for node_name in node_map.keys():
            ...
            for input_idx, input_node_name in enumerate(node.input):
                if (
                    node_map[
                        QuantizeGraphHelper.node_name_from_input(input_node_name)
                    ].op
                    == "Const"
                ):
                    ...
                    if input_node_name in input_map.keys():
                        ...
                        new_input_node.name = (
                            input_node_name + "_" + str(len(input_map[input_node_name]))
                        )
    ...
    @staticmethod
    def remove_training_nodes(
        input_graph, protected_nodes=[], types_to_splice=["Identity", "CheckNumerics"]
    ):
    ...
    def generate_quantized_weight_node(
        ...
        if host_op_type in (
            ...
            if per_channel:
                if host_op_type in ("Conv3D", "Conv3DBackpropInputV2"):
                    ...
                    if (
                        "transpose_b" in input_node.attr
                        and input_node.attr["transpose_b"].b
                    ):  # pragma: no cover
                ...
                if (
                    ...
                    qint8_tensor = (np.around(float_tensor * 127.0 / ranges)).astype(
                        np.int8
                    )
                ...
                qint8_tensor = (np.around(float_tensor * 127.0 / range_value)).astype(
                    np.int8
                )
            ...
            qint8_tensor = (
                np.around(float_tensor.reshape(a, b, c * d) * 127.0 / ranges)
            ).astype(np.int8)
        ...
        min_node = QuantizeGraphHelper.create_constant_node(
            min_name, min_value, dtypes.float32, device="cpu"
        )

        max_node = QuantizeGraphHelper.create_constant_node(
        ...
        if enter_node:
            ...
            QuantizeGraphHelper.set_attr_string(
                qint8_const_enter_node, "frame_name", enter_node.attr["frame_name"].s
            )
            QuantizeGraphHelper.set_attr_dtype(
                qint8_const_enter_node, "T", dtypes.qint8
            )
            QuantizeGraphHelper.set_attr_bool(
                qint8_const_enter_node, "is_constant", True
            )
            ...
            min_enter_node = QuantizeGraphHelper.create_node(
                "Enter", min_name + "_enter", [min_name]
            )
            QuantizeGraphHelper.set_attr_string(
                min_enter_node, "frame_name", enter_node.attr["frame_name"].s
            )
            ...
            max_enter_node = QuantizeGraphHelper.create_node(
                "Enter", max_name + "_enter", [max_name]
            )
            QuantizeGraphHelper.set_attr_string(
                max_enter_node, "frame_name", enter_node.attr["frame_name"].s
            )
