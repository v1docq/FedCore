...
class BF16Convert(GraphRewriterBase):
    ...
    def __init__(self, model, fp32_ops=[], bf16_ops=[]):
        ...
        self.device = [
            "CPU",
            "DEFAULT",
        ]  # TODO support different device types, such as GPU
    ...
    def _dtype_val(self, node):
        ...
        for i in op_def.input_arg:
            ...
            for j in range(inputs_num):
                if i.type:
                    inputs_dt_val.append(
                        copy.deepcopy(attr_value_pb2.AttrValue(type=i.type))
                    )
                else:
                ...
                for constraint in kernel.constraint:
                    if constraint.HasField("allowed_values"):
                        if constraint.name not in registered_dt_val:
                            registered_dt_val[constraint.name] = (
                                constraint.allowed_values.list.type
                            )
                        else:
                            registered_dt_val[constraint.name].extend(
                                constraint.allowed_values.list.type
                            )
    ...
    def _bf16_convert(self, bf16_node_name):
        ...
        for index, input_name in enumerate(bf16_node.input):
            ...
            input_detail = self.cur_graph.node_name_details[
            ...
            if (
                ...
                and dtypes.bfloat16.as_datatype_enum
                not in allowed_dt_val[inputs_dt[index]]
            ):
                continue
            ...
            if (
                ...
                outputs = list(
                    map(lambda x: x.replace(input_name, bf16_node.name), outputs)
                )
                ...
                fp32_value = tensor_util.MakeNdarray(
                    input_node.attr.get("value").tensor
                )
                ...
                if (
                    ...
                    and dtypes.bfloat16.as_datatype_enum
                    in allowed_input_node_dt_val[outputs_dt_input_node[0]]
                ):
            ...
            # ResizeBilinear input can be of different types but output is always float
                ...
                cast_node_name = (
                    input_name.replace(":", "_") + "/" + bf16_node_name + "_FP32toBF16"
                )
                ...
                outputs = list(
                    map(lambda x: x.replace(bf16_node.name, cast_node_name), outputs)
                )
        ...
        for output_name in bf16_node_outputs:
            ...
            for i, input_name in enumerate(output_node.input):
                ...
                if (
                    ...
                    and dtypes.bfloat16.as_datatype_enum
                    not in allowed_dt_val[outputs_dt[index]]
                ):
                    continue
                ...
                if output_node.op == "Cast":
                    ...
                    if (
                        ...
                        and dtypes.bfloat16.as_datatype_enum
                        in allowed_output_node_dt_val["dtype"]
                    ):
                    ...
                    and dtypes.bfloat16.as_datatype_enum
                    not in allowed_output_node_dt_val[inputs_dt_input_node[i]]
                ):
                    cast_node_name = (
                        bf16_node_name + "/" + output_node.name + "_BF16toFP32"
                    )
                    ...
                    self.cur_graph.add_node(
                        output_cast_node, bf16_node_name, [output_name]
                    )
    ...
    def do_transformation(self):
        ...
        converted_graph_def = DequantizeCastOptimizer(
            converted_graph_def
        ).do_transformation()
