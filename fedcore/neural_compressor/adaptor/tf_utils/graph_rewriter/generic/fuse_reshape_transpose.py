    ...
    def do_transformation(self):
        ...
        for i in valid_match:
            ...
            if transpose_input_node.op == "Const":
                ...
                enter_input_node = graph_info[enter_input_node_name].node
                transpose_input_node_content = tensor_util.MakeNdarray(
                    enter_input_node.attr["value"].tensor
                )
            else:
                continue
            ...
            if transpose_input_perm.op == "Const":
                ...
                enter_transpose_input_perm_node = graph_info[
                    enter_transpose_input_perm
                ].node
                transpose_perm_node_content = tensor_util.MakeNdarray(
            ...
            if reshape_shape_node.op == "Const":
                ...
                reshape_shape_node_content = tensor_util.MakeNdarray(
                    enter_reshape_shape_node.attr["value"].tensor
                )
            else:
                continue

            converted_node = transpose_input_node_content.transpose(
                transpose_perm_node_content
            ).reshape(reshape_shape_node_content)
            g.remove_node(i[0])
            if transpose_input_node.op == "Const":
