    ...
    def do_transformation(self):
        ...
        for i in target_nodes:
            ...
            if (
                ...
                and graph_info[Helper.node_name_from_input(next_node_names[0])].node.op
                in valid_ops
            ):
                continue
            bias_node_name = i[0] + "_dummy_biasadd"
            bias_const_node_name = i[0] + "_dummy_biasadd_const"
            matmul_a_node_name = Helper.node_name_from_input(
                graph_info[i[0]].node.input[0]
            )
            matmul_a_node = graph_info[matmul_a_node_name].node
            matmul_b_node_name = Helper.node_name_from_input(
                graph_info[i[0]].node.input[1]
            )
            matmul_b_node = graph_info[matmul_b_node_name].node

            if matmul_a_node.op == "Const" or matmul_b_node.op not in [
                "Const",
                "Enter",
            ]:
                continue
            if matmul_b_node.op == "Enter":  # pragma: no cover
                parent_node = graph_info[
                    Helper.node_name_from_input(matmul_b_node.input[0])
                ].node
            ...
            if graph_info[i[0]].node.op == "MatMul":
                ...
                graph_info[i[0]].node.op == "Conv2D"
                and graph_info[i[0]].node.attr["data_format"].s == b"NHWC"
            ):
                t_b_index = 3
            elif (
                graph_info[i[0]].node.op == "Conv2D"
                and graph_info[i[0]].node.attr["data_format"].s == b"NCHW"
            ):
                t_b_index = 1
            else:
                continue
            ...
            if (
                i[0] in g.parent_frame_details and g.parent_frame_details[i[0]]
            ):  # pragma: no cover
                ...
                Helper.set_attr_string(
                    ...
                    "frame_name",
                    g.parent_frame_details[i[0]].attr["frame_name"].s,
                )
            ...
            bias_node = Helper.create_node(
                ...
                [
                    ...
                    (
                        ...
                        if i[0] in g.parent_frame_details
                        and g.parent_frame_details[i[0]]
            ...
            if (
                ...
                g.add_node(
                    bias_const_enter_node, bias_const_node_name, [bias_node_name]
                )
            else:
