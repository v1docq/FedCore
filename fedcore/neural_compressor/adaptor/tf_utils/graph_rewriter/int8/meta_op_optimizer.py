    ...
    def do_transformation(self):
        ...
        for i in target_nodes:
            ...
            if quant_node.attr["mode"].s.decode() == deq_node.attr["mode"].s.decode():
                ...
                if (
                    self.graph_info[quantized_node_name].node.op
                    == "_QuantizedBatchMatMul"
                    or self.graph_info[quantized_node_name].node.op
                    == "_QuantizedFusedBatchNorm"
                ) and self.graph_info[dequantize_node_name].node.attr[
                    "T"
                ].type != dtypes.qint8.as_datatype_enum:
                    continue

                for index, value in enumerate(
                    self.graph_info[quantized_node_name].node.input
                ):
                ...
                if "Thost_inputs" in self.graph_info[quantized_node_name].node.attr:
                    self.graph_info[quantized_node_name].node.attr[
                        "Thost_inputs"
                    ].list.type[0] = new_dtype

                if "T" in self.graph_info[quantized_node_name].node.attr:
                    self.graph_info[quantized_node_name].node.attr["T"].type = new_dtype

                self.graph_info[i[1]].node.input[0] = self.graph_info[
                    dequantize_node_name
                ].node.input[0]
                self.graph_analyzer.remove_node(dequantize_node_name)
                self.graph_analyzer.remove_node(
                    self.graph_info[quantize_node_name].node.input[1]
                )
                self.graph_analyzer.remove_node(
                    self.graph_info[quantize_node_name].node.input[2]
                )
