...
class GenerateGraphWithQDQPattern(GraphRewriterBase):
        ...
        for op_name in quantizable_op_names:
            ...
            if self.graph_info[op_name].node.op == "ConcatV2":
                if not self.itex_mode:
                    self._insert_qdq_pattern_for_concatv2(
                        self.graph_info[op_name].node, is_asymmetric
                    )
            else:
                self._insert_qdq_pattern_for_common_ops(
                    self.graph_info[op_name].node, is_asymmetric
                )
        ...
        for i in matched_nodes:
            ...
            for index in range(len_deq_outputs - 1):
                rep_dequantize_node = Helper.create_node(
                    ...
                    [
                        quantize_node_name,
                        quantize_node_name + ":1",
                        quantize_node_name + ":2",
                    ],
                )
                ...
                next_node_name = self.g_qdq.node_name_details[deq_node_name].outputs[
                    index + 1
                ]
                self.g_qdq.add_node(
                    rep_dequantize_node, quantize_node_name, [next_node_name]
                )
                for input_index, each_input in enumerate(
                    self.g_qdq.node_name_details[next_node_name].node.input
                ):
    ...
    def _find_relu_node(self, node):
        ...
        if (
            ...
            or self.node_name_mapping[
                Helper.node_name_from_input(node.input[0])
            ].node.op.find("FusedBatchNorm")
            == -1
            or self.node_name_mapping[Helper.node_name_from_input(node.input[0])]
            .node.attr["is_training"]
            .b
            or len(
                self.node_name_mapping[
                    Helper.node_name_from_input(node.input[0])
            ...
            return True
        elif "T" in node.attr and dtypes.DType(node.attr["T"].type) in (
            dtypes.quint8,
            dtypes.uint8,
        ):
            return True
            ...
            if re.search(r"\w+:\d+", node.input[0]):
                input0_node = self.node_name_mapping[
                    node.input[0].rsplit(":", 1)[0]
                ].node
            else:
                input0_node = self.node_name_mapping[node.input[0]].node
            if re.search(r"\w+:\d+", node.input[1]):
                input1_node = self.node_name_mapping[
                    node.input[1].rsplit(":", 1)[0]
                ].node
            else:
                input1_node = self.node_name_mapping[node.input[1]].node
            if input0_node.op in (
                "BiasAdd",
                "Add",
                "AddV2",
                "AddN",
            ) or input1_node.op in (
                "BiasAdd",
                "Add",
                "AddV2",
                "AddN",
                ...
                return False
            return self._find_relu_node(input0_node) and self._find_relu_node(
                input1_node
            )
        elif self._check_op_list(node.op) or (
            self.itex_mode and node.op in ("Add", "AddV2")
        ):
            if node.op == "ConcatV2":
                find_relu = False
                for i in range(0, node.attr["N"].i):
                    if re.search(r"\w+:\d+", node.input[i]):
                        input_node = self.node_name_mapping[
                            node.input[i].rsplit(":", 1)[0]
                        ].node
                    else:
    ...
    def _insert_qdq_pattern_for_common_ops(self, original_node, is_asymmetric):
        ...
        for each_input_name in all_inputs:
            ...
            if (
                ...
                and self.graph_info[
                    self.graph_info[each_input_name].node.input[0]
                ].node.op
                == "Dequantize"
            ):
                maxpool_node = self.graph_info[each_input_name].node
                dtype = dtypes.DType(
                    self.graph_info[maxpool_node.input[0]].node.attr["T"].type
                )
            ...
            # the qdq in pattern dq+bn+relu+q and dq+bn+q should be s8 in itex mode
                ...
                self.node_name_mapping[original_node.name].node.op == "FusedBatchNormV3"
            ):
                dtype = dtypes.qint8
            else:
    ...
    def _insert_qdq_pattern_for_each_input(
        ...
        if self.fake_quant:  # pragma: no cover
            min_node = Helper.create_constant_node(
                min_input_name, -1.0, dtypes.float32, device="cpu"
            )
            max_node = Helper.create_constant_node(
                max_input_name, 1.0, dtypes.float32, device="cpu"
            )
            ...
            if "BatchMatMul" in self.graph_info[op_name].node.op:
                ...
                Helper.set_attr_string(
                    dequantize_node,
                    "mode",
                    b"MIN_FIRST" if is_asymmetric else b"SCALED",
                )
            ...
            if (
                self.itex_mode
                and self.graph_info[op_name].node.op == "FusedBatchNormV3"
            ):
                ...
                max_input_name = namespace_prefix + "_max_" + unique_input_name
                quantize_input_name = (
                    namespace_prefix + "_quantize_" + unique_input_name
                )

            reshape_input_node = Helper.create_node(
                "Reshape", reshape_input_name, [input_name, reshape_dims_name]
            )
            ...
            if (
                ...
                or "BatchMatMul" in self.graph_info[op_name].node.op
            ):
                Helper.set_attr_string(quant_v2_node, "mode", b"SCALED")
            else:
                Helper.set_attr_string(
            ...
            if self.graph_info[op_name].node.op in (
                "Conv2DBackpropInput",
                "Conv3DBackpropInputV2",
            ):
                input_index = 2
            ...
            self.g.add_node(
                reshape_input_node, reshape_dims_name, [min_input_name, max_input_name]
            )
    ...
    def _insert_qdq_pattern_for_weight_node(
        ...
        if "value" in weight_node.attr and host_op_type in (
            ...
            if per_channel:
                if host_op_type in ("Conv3D", "Conv3DBackpropInputV2"):
                    ...
                    if (
                        "transpose_b" in weight_node.attr
                        and weight_node.attr["transpose_b"].b
                    ):  # pragma: no cover
        ...
        min_node = Helper.create_constant_node(
            min_name, min_value, dtypes.float32, device="cpu"
        )
        max_node = Helper.create_constant_node(
            max_name, max_value, dtypes.float32, device="cpu"
        )
        ...
        if insert_reshape:
            ...
            self.g_weight.add_node(
                reshape_4to3_node, weight_node.name, [quant_node.name]
            )
            ...
            self.g_weight.add_node(
                reshape_3to4_node, dequant_node.name, [computational_node.name]
            )
            ...
            if (
                ...
                weight_enter_node = Helper.create_node(
                    "Enter", weight_node.name + "_enter", [weight_node.name]
                )
                Helper.set_attr_string(
                    weight_enter_node,
                    "frame_name",
                ...
                min_enter_node = Helper.create_node(
                ...
                max_enter_node = Helper.create_node(
                ...
                self.g_weight.add_node(
                    weight_enter_node, weight_node.name, [quant_node.name]
                )
                ...
                self.g_weight.add_node(
                    dequant_node, quant_node.name, [computational_node.name]
                )
    ...
    def _ignore_insert_qdq_pattern(self, matched_node_name):
        ...
        if (
            matched_node_name in self.fp32_ops or matched_node_name in self.bf16_ops
        ) and ((matched_node_name,) not in self.quantized_nodes):
            return True

        if (
            matched_node_name not in self.op_wise_config
            and (matched_node_name,) not in self.quantized_nodes
        ):
            return True
        ...
        if (
            not self.itex_mode
            and self.graph_info[matched_node_name].node.op == "MatMul"
        ):
            if self.graph_info[matched_node_name].node.attr["transpose_a"].b is True:
                return True
        if (
            "FusedBatchNorm" in self.graph_info[matched_node_name].node.op
            and not self.itex_mode
        ):
            return True
        if (
            "_MklFusedInstanceNorm" == self.graph_info[matched_node_name].node.op
            and not self.itex_mode
        ):
            return True
        return False
