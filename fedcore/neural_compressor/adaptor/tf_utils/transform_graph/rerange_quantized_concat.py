        ...
        if op_type == "QuantizedConcatV2":
            ...
            for index in range(concat_input_num):
                ...
                if concat_input_node_op_type in self.offset_map:
                    ...
                    and str(concat_input_node.attr["fused_ops"].list.s)
                    in self.offset_map_new_api
                ):
                    quantized_conv_nodes.append(concat_input_node)
                elif concat_input_node_op_type in (
                    "QuantizedMaxPool",
                    "QuantizedAvgPool",
                ):
                    another_concat_node = self.node_mapping[
                        self.get_node_name_from_input(concat_input_node.input[0])
                    ]
                    if self.performance_only:
                        if (
                            another_concat_node.op == "_FusedQuantizedConv2D"
                            and "fused_ops" in another_concat_node.attr
                            and str(another_concat_node.attr["fused_ops"].list.s)
                            in self.offset_map_new_api
                        ):
                    ...
                    self.performance_only and concat_input_node_op_type == "QuantizeV2"
                ):
                    quantized_conv_nodes.append(concat_input_node)
                else:
                    can_rerange = False
            ...
            and input_node.attr["fused_ops"].list.s
            in self.fuse_requantized_relu_op_new_api
        ):
            can_rerange = True
        ...
        for _, node in enumerate(self.input_graph.node):
            ...
            for node in quantized_conv_nodes:
                ...
                if (
                    ...
                    offset_value = self.offset_map_new_api[
                        str(node.attr["fused_ops"].list.s)
                    ]
    ...
    def _update_bias(self):
        ...
        for node_name in self.node_mapping:
            ...
            if (current_node_op in self.fused_requantized_bias_op) or (
                ...
                and current_node.attr["fused_ops"].list.s
                in self.fuse_requantized_bias_op_new_api
            ):
                done = False
                ...
                while not done:
                    ...
                    if current_node.op in self.offset_map:
                        ...
                        and "fused_ops" in current_node.attr
                        and str(current_node.attr["fused_ops"].list.s)
                        in self.offset_map_new_api
                    ):
                        another_conv_node = current_node
                        done = True
                ...
                bias_node = self.node_mapping[
                    self.get_node_name_from_input(original_conv_node.input[2])
                ]
                bias_node_type = original_conv_node.attr["Tbias"]

                if (
                    bias_node_type.type != dtypes.float32
                    or bias_node_type.type == dtypes.qint32
                ):
                    continue
                sum_off_set = 0
                if original_conv_node.op == "_FusedQuantizedConv2D":
                    if str(original_conv_node.attr["fused_ops"].list.s) == str(
                        [b"BiasAdd", b"Sum", b"Relu", b"Requantize"]
                    ) or str(original_conv_node.attr["fused_ops"].list.s) == str(
                        [b"BiasAdd", b"Sum", b"Requantize"]
                    ):
                        sum_off_set = 1
                    # else:
                ...
                min_filter_node = self.node_mapping[
                    original_conv_node.input[5 + sum_off_set]
                ]
                max_filter_node = self.node_mapping[
                    original_conv_node.input[6 + sum_off_set]
                ]
                ...
                if (
                    ...
                    and str(another_conv_node.attr["fused_ops"].list.s)
                    in self.offset_map_new_api
                ):
                    offset_value = self.offset_map_new_api[
                        str(another_conv_node.attr["fused_ops"].list.s)
                    ]
                else:
                ...
                activation_range = (
                    127.0
                    if current_node.attr["out_type"].type == dtypes.qint8
                    else 255.0
                )
                ...
                if "Thost_inputs" in original_conv_node.attr:
                    original_conv_node.attr["Thost_inputs"].list.type[2] = (
                        original_conv_node.attr["Tbias"].type
                    )
