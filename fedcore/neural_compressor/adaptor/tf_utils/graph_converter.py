...
from fedcore.neural_compressor.adaptor.tf_utils.graph_rewriter.generic.insert_print_node import (
    InsertPrintMinMaxNode,
)
from fedcore.neural_compressor.conf.dotdict import deep_get
from fedcore.neural_compressor.model import Model
from fedcore.neural_compressor.model.tensorflow_model import TensorflowSavedModelModel
...
from .graph_rewriter.generic.fuse_pad_with_fp32_conv import (
...
from .graph_rewriter.int8.fuse_conv_redundant_dequantize import (
    FuseConvRedundantDequantizeTransformer,
)
from .graph_rewriter.int8.fuse_conv_requantize import FuseConvRequantizeTransformer
from .graph_rewriter.int8.fuse_matmul_redundant_dequantize import (
...
class GraphConverter:
    ...
    def __init__(
            ...
            self._fp32_model = Model(
                ...
                **self.model.kwargs,
                backend=(
                    "itex"
                    if itex_mode
                    and not isinstance(self.model, TensorflowSavedModelModel)
                    else "default"
                )
            )
        ...
        self.new_api = (
            new_api  # bool(version1_gte_version2(tf.version.VERSION, '2.8.0'))
        )
    ...
    def _inference(self, model):
        ...
        if hasattr(self.data_loader, "__len__") and len(self.data_loader) == 0:
            ...
            _ = (
                ...
                if iter_op == []
                else iterator_sess_run(
                    sess, iter_op, feed_dict, output_tensor, self.calib_iteration
                )
            )
    ...
    def _inference_llm(self, model):
        ...
        for idx, (inputs, _) in enumerate(self.data_loader):
            ...
            if len(input_tensor_names) == 1:
                ...
                assert len(input_tensor_names) == len(
                    inputs
                ), "inputs len must equal with input_tensor"
    ...
    def _check_tf_version(self):
        ...
        try:
            ...
            if IsMklEnabled() and (
                version1_lte_version2(TF_SUPPORTED_MIN_VERSION, tf.version.VERSION)
            ):
            ...
            if (
                version1_gt_version2(tf.version.VERSION, TF_SUPPORTED_MAX_VERSION)
                and not is_sprbase_version
            ):
                logger.warning(
                    str(
            ...
            if (
                version1_eq_version2(tf.version.VERSION, "2.5.0")
                and os.getenv("TF_ENABLE_MKL_NATIVE_FORMAT") != "0"
            ):
                logger.fatal(
                    "Please set environment variable TF_ENABLE_MKL_NATIVE_FORMAT=0 "
            ...
            if not is_supported_version:
                raise ValueError(
                    str(
                        "Please install TensorFlow within version >={} and <={}."
    ...
    def _gen_tmp_filenames(self):
        ...
        self._int8_dynamic_range_model_path = os.path.join(
            self._output_path, "int8_dynamic_range_graph"
        )
        self._int8_logged_model_path = os.path.join(
            self._output_path, "int8_logged_graph"
        )
        self._fp32_logged_model_path = os.path.join(
            self._output_path, "fp32_logged_graph"
        )
    ...
    def convert(self):
        ...
        if self.itex_mode:
            host_const_graph_def = PostHostConstConverter(
        ...
        if self.new_api:
            if self.performance_only:
                model.graph_def = FuseConvRedundantDequantizeTransformer(
                    model.graph_def
                ).do_transformation()
            post_optimize_graph_def = FuseMatMulRedundantDequantizeTransformer(
                model.graph_def
            ).do_transformation()
        ...
        post_hostconst_graph_def = PostHostConstConverter(
            post_cse_graph_def
        ).do_transformation()
    ...
    def _get_fp32_print_node_names(self, specified_op_list):
        ...
        fp32_node_name_mapping = {
            node.name: node for node in sorted_graph.node if node.op != "Const"
        }
        ...
        for i in target_conv_op:
            if specified_op_list and i not in specified_op_list:
                continue
            if (
                node_name_mapping[i + "_eightbit_quantized_conv"].op
                ...
                end_index = (
                    start_index
                    + offset_map[node_name_mapping[i + "_eightbit_quantized_conv"].op]
                )
            ...
            if self.fake_quant:  # pragma: no cover
                ...
                if self._enable_kl_op_names:
                    ...
                    self._generate_calibration_data(
                        self._fp32_logged_model_path, self._fp32_print_data, True
                    )
                ...
                if self.quantized_node_info:
                    ...
                    self._calibration_data = Helper.gen_valid_sampling_log(
                        tmp_dump_file
                    )
    ...
    def _quantize_graph(self):
        ...
        self._tmp_graph_def, self.quantized_node_info, exclude_node_names = (
    ...
    def _generate_calibration_data(self, tmp_path, output_data, enable_kl_algo=False):
        ...
        tmp_dump_file = os.path.join(
            os.path.dirname(self.output_graph), "requant_min_max.log"
        )
            ...
            model = Model(
                ...
                **self._tmp_model.kwargs,
                backend=(
                    "itex"
                    if self.itex_mode
                    and not isinstance(self._tmp_model, TensorflowSavedModelModel)
                    else "default"
                )
            )
        ...
        for line in output_data:
            if enable_kl_algo and line.rsplit(":")[0] in self._kl_keys:
                ...
                key = (
                    self._print_node_mapping[line[1:].split("__print")[0]]
                    + "_eightbit_requant_range"
                )
    ...
    def _freeze_requantization_ranges(self, additional_data=None):
        ...
        if (
            "scale_propagation_max_pooling" in self.recipes
            and self.recipes["scale_propagation_max_pooling"]
        ):
    ...
    def _fuse_requantize_with_fused_quantized_node(self):
        ...
        input_output_names = (
            self._tmp_model.input_node_names + self._tmp_model.output_node_names
        )
        ...
        if self.performance_only or (
            "scale_propagation_concat" in self.recipes
        ...
        if (
            self.advance_config is not None
            and deep_get(self.advance_config, "bias_correction") is not None
        ):
    ...
    def _post_clean(self):
        ...
        :return: None
        """
        if os.path.exists(self._int8_logged_model_path) and os.path.isdir(
    ...
    def _convert_qdq(self):
        ...
        if self.itex_mode:
            ...
            self._tmp_graph_def = ShareQDQForItexYPatternOptimizer(
                self._tmp_graph_def
            ).do_transformation()
            self._tmp_graph_def = MergeDuplicatedQDQOptimizer(
                self._tmp_graph_def
            ).do_transformation()
