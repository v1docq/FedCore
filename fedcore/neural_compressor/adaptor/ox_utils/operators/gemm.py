...
class GemmOperator(Operator):
    ...
    def quantize_check(self):
        ...
        node = self.node
        if len(node.input) == 3 and not find_by_name(
            node.input[2], self.quantizer.model.initializer()
        ):
    ...
    def quantize(self):
        ...
        if len(node.input) == 3 and find_by_name(
            node.input[2], self.quantizer.model.initializer()
        ):
    ...
    def convert(self, convert_format):
        ...
        qgemm_node = onnx.helper.make_node(
            "QGemm", qgemm_inputs, [qgemm_output], node.name, **kwargs
        )
...
class QGemmOperator(QOperator):
    ...
    def convert(self):
        ...
        bias_scale_initializer = onnx.numpy_helper.from_array(
            bias_scale_data, node.input[6] + "_scale"
        )
        ...
        gemm_node = onnx.helper.make_node(
            "Gemm", inputs, outputs, node.name + "_convert", **kwargs
        )
