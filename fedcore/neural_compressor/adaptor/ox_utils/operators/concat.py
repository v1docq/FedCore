...
class ConcatOperator(Operator):
    ...
    def quantize_check(self):
        ...
        if all(
            [
                inp not in self.quantizer.quantized_value_map and inp not in inits
                for inp in node.input
            ]
        ) or not all(
            [
                inp in self.quantizer.quantized_value_map or inp in inits
                for inp in node.input
            ]
        ):
            return False
    ...
    def convert_check(self, convert_format):
        ...
        if all([i.op_type == "DequantizeLinear" for i in parents]) and any(
            ...
            if (
                any(
                    [
                        self.quantizer.model.get_initializer(zp) is None
                        for zp in input_zp
                    ]
                )
                or any(
                ...
                or any(
                    [
                        self.quantizer.model.get_initializer(scale) is None
            ...
            if any(
                [
                    self.quantizer.model.get_initializer(scale).data_type != 1
                    for scale in input_scale
                ]
            ):  # pragma: no cover
                return False
            ...
            if any(
                [
                    ...
                    not in [
                        self.quantizer.model.get_initializer(out_zp).data_type
                        for out_zp in output_zp
                    ]
...
class QConcatOperator(QOperator):
    ...
    def convert(self):
        ...
        concat_node = onnx.helper.make_node(
            "Concat", inputs, outputs, node.name + "_convert", **kwargs
        )
