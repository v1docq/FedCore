...
def _valid_accuracy_field(key, scope, error):
    assert bool("relative" in scope["accuracy_criterion"]) != bool(
...
def percent_to_float(data):
    ...
    if isinstance(data, int):
        ...
        assert isinstance(
            data, float
        ), "This field should be float, int or percent string"
    return data
...
ops_schema = Schema(
    {
        ...
        Optional("activation", default=None): {
            ...
            Optional("compute_dtype", default=["uint8"]): And(
                list,
                lambda s: all(
                    i in ["int8", "uint8", "fp32", "bf16", "None"] for i in s
                ),
            ),
...
filter_schema = Schema(
    {
        Optional("LabelBalance"): {"size": And(int, lambda s: s > 0)},
    }
)
...
transform_schema = Schema(
    {
        ...
        Optional("Cast"): {Optional("dtype"): str},
        Optional("RandomResizedCrop"): {
            "size": Or(
                And(list, lambda s: all(isinstance(i, int) for i in s)),
                And(int, lambda s: s > 0),
            ),
            Optional("scale"): And(
                list, lambda s: all(isinstance(i, float) for i in s)
            ),
            Optional("ratio"): And(
                list, lambda s: all(isinstance(i, float) for i in s)
            ),
            Optional("interpolation"): And(
                str, lambda s: s in ["nearest", "bilinear", "bicubic"]
            ),
        },
        Optional("AlignImageChannel"): {Optional("dim"): int},
        ...
        Optional("QuantizedInput"): {
            Optional("dtype", default="int8"): And(
                str, lambda s: s in ["int8", "uint8"]
            ),
        ...
        Optional("ResizeCropImagenet"): {
            ...
            Optional("resize_method", default="bilinear"): And(
                str,
                lambda s: s
                in [
                    "bilinear",
            ...
            Optional("data_format", default="channels_last"): And(
                str, lambda s: s in ["channels_first", "channels_last"]
            ),
            Optional("subpixels", default="RGB"): And(
                str, lambda s: s in ["BGR", "RGB"]
            ),
            Optional("mean_value"): And(Or(str, list), Use(input_to_list_float)),
            Optional("scale"): float,
        },
...
dataset_schema = Schema(
    {
        ...
        Optional("dummy_v2"): {
            ...
            Optional("label_shape", default=[1]): And(
                Or(str, list), Use(list_to_tuple)
            ),
            Optional("low"): Or(
                float,
...
dataloader_schema = Schema(
    {
        Optional("last_batch", default="rollover"): And(
            str, lambda s: s in ["rollover", "discard"]
        ),
...
criterion_schema = Schema(
    {
        Optional("CrossEntropyLoss"): {
            Optional("reduction", default="mean"): And(
                str,
                lambda s: s in ["none", "sum", "mean", "auto", "sum_over_batch_size"],
            ),
            Optional("from_logits", default=False): And(
                bool, lambda s: s in [True, False]
            ),
        },
        ...
        Optional("KnowledgeDistillationLoss"): {
            ...
            Optional("loss_types"): And(
                list, lambda s: all(i in ["CE", "KL", "MSE"] for i in s)
            ),
            Optional("loss_weights"): And(
                list, lambda s: all(i >= 0 for i in s) and sum(s) == 1.0
            ),
        },
        Optional("IntermediateLayersKnowledgeDistillationLoss"): {
            "layer_mappings": And(
                Or(tuple, list), lambda s: all(len(i) in [1, 2] for i in s)
            ),
            Optional("loss_types"): And(
                Or(tuple, list), lambda s: all(i in ["MSE", "KL", "L1"] for i in s)
            ),
            Optional("loss_weights"): And(
                Or(tuple, list), lambda s: all(i >= 0 for i in s)
            ),
            Optional("add_origin_loss"): bool,
        },
        Optional("SelfKnowledgeDistillationLoss"): {
            "layer_mappings": And(
                Or(tuple, list), lambda s: all(len(i) >= 1 for i in s)
            ),
            Optional("loss_types"): And(
                Or(tuple, list), lambda s: all(i in ["L2", "CE", "KL"] for i in s)
            ),
            Optional("loss_weights"): And(
                Or(tuple, list), lambda s: all(i >= 0.0 and i < 1.0 for i in s)
            ),
            Optional("add_origin_loss"): bool,
            Optional("temperature"): And(float, lambda s: s > 0),
        },
...
train_schema = Schema(
    {
        ...
        Optional(
            "optimizer", default={"SGD": {"learning_rate": 0.001}}
        ): optimizer_schema,
        ...
        Optional("execution_mode", default="eager"): And(
            str, lambda s: s in ["eager", "graph"]
        ),
        Optional("postprocess"): {Optional("transform"): postprocess_schema},
...
weight_compression_schema = Schema(
    {
        Optional("initial_sparsity", default=0): And(
            float, lambda s: s < 1.0 and s >= 0.0
        ),
        ...
        Optional("pruners"): And(list, lambda s: all(isinstance(i, Pruner) for i in s)),
    }
)
...
COCOmAP_input_order_schema = Schema(
    {Optional("num_detections"): int, "boxes": int, "scores": int, "classes": int}
)
...
schema = Schema(
    {
        ...
        Optional("version", default=float(__version__.split(".")[0])): Or(
            float, And(int, Use(input_int_to_float)), And(str, Use(input_int_to_float))
        ),
        ...
        Optional(
            ...
            default={
                ...
                "model_wise": {"weight": {"bit": [7.0]}, "activation": {}},
                "quant_level": "auto",
            },
        ): {
            ...
            Optional("advance", default=None): {
                Optional("bias_correction"): And(
                    str, lambda s: s in ["weight_empirical"]
                ),
            },
            Optional("calibration", default={"sampling_size": [100]}): {
                Optional("sampling_size", default=[100]): And(
                    Or(str, int, list), Use(input_to_list)
                ),
            ...
            Optional(
                ...
                Optional("scale_propagation_max_pooling", default=True): And(
                    bool, lambda s: s in [True, False]
                ),
                Optional("scale_propagation_concat", default=True): And(
                    bool, lambda s: s in [True, False]
                ),
                Optional("first_conv_or_matmul_quantization", default=True): And(
                    bool, lambda s: s in [True, False]
                ),
                Optional("last_conv_or_matmul_quantization", default=True): And(
                    bool, lambda s: s in [True, False]
                ),
                Optional("pre_post_process_quantization", default=True): And(
                    bool, lambda s: s in [True, False]
                ),
                Optional("fast_bias_correction", default=False): And(
                    bool, lambda s: s in [True, False]
                ),
                Optional("weight_correction", default=False): And(
                    bool, lambda s: s in [True, False]
                ),
            },
            Optional(
                "model_wise", default={"weight": {"bit": [7.0]}, "activation": {}}
            ): {
                ...
                Optional("activation", default=None): {
                    ...
                    Optional("algorithm", default=None): And(
                        ...
                        Use(input_to_list),
                        lambda s: all(
                            i in ["minmax", "kl", "placeholder", "percentile"]
                            for i in s
                        ),
                    ),
                },
            },
            Optional("optype_wise", default=None): {str: ops_schema},
            Optional("op_wise", default=None): {str: ops_schema},
            Optional("quant_level", default="auto"): And(
                Or(str, int), lambda level: level in ["auto", 0, 1]
            ),
        },
        ...
        Optional(
            ...
            Optional("multi_objectives"): {
                Optional("objective"): And(
                    Or(str, list),
                    Use(input_to_list),
                    lambda s: all(i in OBJECTIVES for i in s),
                ),
            ...
            Optional(
                "exit_policy",
                default={"timeout": 0, "max_trials": 100, "performance_only": False},
            ): {
            ...
            Optional("tensorboard", default=False): And(
                bool, lambda s: s in [True, False]
            ),
            ...
            Optional("diagnosis", default=False): And(
                bool, lambda s: s in [True, False]
            ),
        },
        Optional("evaluation"): {
            ...
            Optional("accuracy"): {
                ...
                Optional("multi_metrics", default=None): {
                    ...
                    Optional("mAP"): {
                        ...
                        Optional("iou_thrs", default=0.5): Or(
                            And(str, lambda s: s in ["0.5:0.05:0.95"]),
                            And(float, lambda s: s <= 1.0 and s >= 0.0),
                        ),
                        Optional("map_points", default=0): And(
                            int, lambda s: s in [0, 11, 101]
                        ),
                    },
                    Optional("COCOmAP"): {
                        Optional("anno_path"): str,
                        Optional(
                            "map_key", default="DetectionBoxes_Precision/mAP"
                        ): str,
                    },
                    Optional("COCOmAPv2"): {
                        Optional("anno_path"): str,
                        ...
                        ): COCOmAP_input_order_schema,
                    },
                    Optional("VOCmAP"): {Optional("anno_path"): str},
                    Optional("SquadF1"): Or({}, None),
                    Optional("MSE"): {Optional("compare_label"): bool},
                    Optional("RMSE"): {Optional("compare_label"): bool},
                    Optional("MAE"): {Optional("compare_label"): bool},
                    Optional("Accuracy"): Or({}, None),
                    Optional("Loss"): Or({}, None),
                    Optional("BLEU"): Or({}, None),
                    Optional("SquadF1"): Or({}, None),
                    Optional("F1"): Or({}, None),
                    Optional("mIOU"): {Optional("num_classes"): int},
                    Optional("GLUE"): {Optional("task"): str},
                    Optional("ROC"): {Optional("task"): str},
                },
        ...
        Optional("distillation"): {Optional("train"): train_schema},
    ...
    def _read_cfg(self, cfg_fname):
        ...
        try:
            ...
            if "version" not in cfg:
                leading_whitespace = (
                    re.search(r"[ \t]*model\s*:", content).group().split("model")[0]
                )
                content = re.sub(
                    r"model\s*:",
                    "version: {}\n\n{}model:".format(
                        float(__version__.split(".")[0]), leading_whitespace
                    ),
                    content,
                )
    ...
    def map_pyconfig_to_cfg(self, pythonic_config):
            ...
            mapping.update(
                {
                    ...
                    "quantization.calibration.sampling_size": pythonic_config.quantization.calibration_sampling_size,
                    ...
                    "tuning.accuracy_criterion.relative": pythonic_config.quantization.accuracy_criterion.relative,
                    "tuning.accuracy_criterion.absolute": pythonic_config.quantization.accuracy_criterion.absolute,
                    "tuning.accuracy_criterion.higher_is_better": pythonic_config.quantization.accuracy_criterion.higher_is_better,
        ...
        if pythonic_config.benchmark is not None:
            ...
            mapping.update(
                {
                    ...
                    "evaluation.performance.configs.cores_per_instance": pythonic_config.benchmark.cores_per_instance,
                    "evaluation.performance.configs.num_of_instance": pythonic_config.benchmark.num_of_instance,
                    "evaluation.performance.configs.inter_num_of_threads": pythonic_config.benchmark.inter_num_of_threads,
                    "evaluation.performance.configs.intra_num_of_threads": pythonic_config.benchmark.intra_num_of_threads,
                    "evaluation.accuracy.configs.cores_per_instance": pythonic_config.benchmark.cores_per_instance,
                    "evaluation.accuracy.configs.num_of_instance": pythonic_config.benchmark.num_of_instance,
                    "evaluation.accuracy.configs.inter_num_of_threads": pythonic_config.benchmark.inter_num_of_threads,
                    "evaluation.accuracy.configs.intra_num_of_threads": pythonic_config.benchmark.intra_num_of_threads,
                }
            )
            if pythonic_config.benchmark.diagnosis:
                mapping.update(
                    {"evaluation.diagnosis": pythonic_config.benchmark.diagnosis}
                )
            ...
            if "model.backend" not in mapping:
                ...
                if (
                    mapping["model.backend"] == "default"
                    and pythonic_config.benchmark.backend != "default"
                ):
                    mapping.update(
                        {
                            "model.backend": pythonic_config.benchmark.backend,
        ...
        for k, v in mapping.items():
            if k in [
                ...
                if target_key not in k and "accuracy_criterion" in self.usr_cfg.tuning:
                    if (
                        target_key in self.usr_cfg.tuning.accuracy_criterion
                        and k.split(".")[-1] in self.usr_cfg.tuning.accuracy_criterion
                    ):
            ...
            if key in dst:
                if isinstance(dst[key], dict) and isinstance(src[key], dict):
                    if key in [
                        ...
                        inter_key = src[key].keys() & dst[key].keys() - {
                            "higher_is_better"
                        }
                        if len(inter_key) == 0:
                            dst[key] = {}
        ...
        for optype in model_wise_quant.keys():
            if cfg.quantization.optype_wise and optype in cfg.quantization.optype_wise:
                self._model_wise_tune_space[optype] = self._merge_dicts(
                    cfg.quantization.optype_wise[optype], model_wise_quant[optype]
                )
            else:
                self._model_wise_tune_space[optype] = self._merge_dicts(
                    cfg.quantization.model_wise, model_wise_quant[optype]
                )
            ...
            self.usr_cfg = DotDict(
                schema.validate(
                    self._convert_cfg(
                        cfg,
                        copy.deepcopy(
                            graph_optimization_default_schema.validate(dict())
                        ),
                    )
                )
            )
...
class NASConfig(Conf):
    ...
    def __init__(self, approach=None, search_space=None, search_algorithm=None):
        assert approach is None or (
            isinstance(approach, str) and approach
