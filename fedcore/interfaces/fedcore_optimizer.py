...
from fedcore.repository.constanst_repository import FEDCORE_MUTATION_STRATEGY, FEDCORE_GRAPH_VALIDATION
...
class FedcoreDispatcher(MultiprocessingDispatcher):
    def __init__(self, **kwargs):
        ...
        self.parallel_backednd = Parallel(n_jobs=self.n_jobs, verbose=0,
    ...
    def _eval_at_least_one_ind(self, individuals):
        for single_ind in individuals:
            evaluation_result = self.fedcore_evaluate_single(self, graph=single_ind.graph,
    ...
    def evaluate_population(self, individuals: PopulationT) -> PopulationT:
        ...
        with parallel_backend(backend='dask',
            ...
            evaluation_results = list(map(lambda ind:
                                          self.fedcore_evaluate_single(self, graph=ind.graph,
            ...
            successful_evals = Maybe(
                value=individuals,
                monoid=not successful_evals).maybe(default_value=successful_evals,
    ...
    def fedcore_evaluate_single(self,
        ...
        if self._n_jobs != 1:
            OperationTypesRepository = FedcoreModels().setup_repository()
...
class FedcoreEvoOptimizer(EvoGraphOptimizer):
    def __init__(self,
        ...
        graph_optimizer_params.adaptive_mutation_type = RandomAgent(actions=graph_optimizer_params.mutation_types,
                                                                    probs=FEDCORE_MUTATION_STRATEGY[
        ...
        self.eval_dispatcher = FedcoreDispatcher(adapter=graph_generation_params.adapter,
